...
in patients undergoing painful and expensive surgeries that were not even necessary. These type of cancers are called 
**benign** which do not require surgeries and we can reduce these unnecessary surgeries by using Machine Learning. 
I have taken the dataset of the previous breast cancer patients and trained the model to predict whether the cancer is **benign** or **malignant**. These predictions will help doctors to do surgeries only when the cancer is malignant, thus reducing the unnecessary surgeries for women. 
 
For building the project I have used [Wisconsin Breast Cancer data](https://archive.ics.uci.edu/ml/datasets/breast+cancer+wisconsin+(diagnostic)) which has 569 rows of which 357 are benign and 212 are malignant. 
The data is preprocessed and scaled. I have trained with Random Forest Classifier which gives the best accuracy of 95.0%. To provide an easy-to-use interface to doctors, I have developed a website that will take the data and display the output with accuracy and time taken to predict.
...
## Project Setup
  ...
  * Create a [virtual environment](https://packaging.python.org/en/latest/guides/installing-using-pip-and-virtual-environments/) for the project. 
  ```sh
  # Using conda
...
Once you have cloned the `Breast-Cancer-Predictor` repository from GitHub, move to that folder first using change directory command on Linux, Mac, and Windows
```sh
# This will change directory to Breast-Cancer-Predictor
...
origin    https://github.com/Your_Username/Breast-Cancer-Predictor.git (fetch)
origin    https://github.com/Your_Username/Breast-Cancer-Predictor.git (push)
upstream  https://github.com/srimani-programmer/Breast-Cancer-Predictor.git (fetch)
upstream  https://github.com/srimani-programmer/Breast-Cancer-Predictor.git (push)
```
...
$ git checkout -b New_Pipeline origin/New_Pipeline

# Reset local `main` branch to match the `upstream` repository's `main` branch(here main branch is named master)
